...
export default function CommercialOtherInformation() {
  ...
  // Pre-fill local state if context already contains saved otherInformation data
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    // 1. Persist Step 3 data into the shared outlet context
    updateFormData("otherInformation", formData);

    // 2. Build final payload combined with previous steps
    const finalPayload = {
      ...contextData,
      otherInformation: formData,
    };

    console.log("Final Commercial Property Payload Submitted:", finalPayload);

    // 3. Navigate to dashboard or target route after submit
    navigate("/dashboard");
  };
