...
export default function CommercialPropertyDetails({ onNext, onPrevious }) {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif' }}>
      <h3 style={{ textAlign: 'center', color: '#1a5b8c', marginBottom: '4px' }}>COMMERCIAL PROPERTY</h3>
      <p style={{ textAlign: 'center', color: '#666', marginTop: '0', fontSize: '14px' }}>Property Details</p>
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
          {AMENITIES_LIST.map((amenity, idx) => (
            <label key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', color: '#444' }}>
              <input
                type="checkbox"
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        <p style={{ color: '#dc3545', fontSize: '12px', marginTop: '8px' }}>
          Please upload min 4 files for image.<br />
          ...
          {NEARBY_LOCATIONS.map((item) => (
            <div key={item.key}>
              ...
              <input
                ...
                style={{ width: '100%', border: 'none', borderBottom: '1px solid #ccc', padding: '6px 0', outline: 'none', fontSize: '13px' }}
              />
            </div>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', gap: '15px', marginTop: '40px' }}>
        <button
          type="button"
          onClick={onPrevious}
          style={{ backgroundColor: '#8cc63f', color: '#fff', border: 'none', padding: '10px 30px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}
        >
          Previous
        </button>
        <button
