...
function EditLeadForm() {
  ...
  // Helper function to format ISO dates to YYYY-MM-DD for <input type="date">
  ...
  useEffect(() => {
    const fetchLead = async () => {
      try {
        setFetching(true);
        const response = await axios.get(`${API_BASE}/getbyid/${id}`);
        ...
        if (data) {
          setFormData({
            ...
            jointApplicantDateOfBirth: formatDateForInput(
              data.jointApplicantDateOfBirth,
            ),
            probableDateForAgreement: formatDateForInput(
              data.probableDateForAgreement,
            ),
            probableDateOfRegistry: formatDateForInput(
              data.probableDateOfRegistry,
            ),
            bookingDate: formatDateForInput(data.bookingDate),
          });

          // Store paths or URLs of existing files if available
          setExistingFiles({
            bookingImageFile: data.bookingImageFile || null,
            bookingFormFile: data.bookingFormFile || null,
            saleConfirmationDocFile: data.saleConfirmationDocFile || null,
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validate()) {
      setTimeout(scrollToTop, 50); // Ensure validation error messages render first
      return;
    }
    ...
    try {
      ...
      setSubmitMessage({
        type: "success",
        text: response?.data?.message || "Booked lead updated successfully!",
      });
    } catch (error) {
      console.error("Error updating lead:", error);
      setSubmitMessage({
        type: "error",
        text: error.response?.data?.message || "Failed to update booked lead.",
      });
    } finally {
      setLoading(false);
      // Timeout gives React time to render the alert banner before scrolling
  ...
  return (
    <div className={styles.container} ref={formContainerRef}>
      <div className={styles.header}>Edit Booked Lead Form</div>
        ...
        <div className={styles.row}>
          ...
          <div className={styles.fieldContainer}>
            <input
              ...
              onChange={handleChange}
              placeholder="Joint Applicant's Alt Mobile No"
              className={styles.input}
            />
          </div>
        </div>
        ...
        <div className={styles.row}>
          ...
          <div className={styles.fieldContainer}>
            <div className={styles.fileContainer}>
              ...
              />
              {existingFiles.bookingImageFile && (
                <div
                  style={{ fontSize: "11px", color: "#555", marginTop: "4px" }}
                >
                  Current file: {existingFiles.bookingImageFile}
                </div>
              )}
          ...
          <div className={styles.fieldContainer}>
            <div className={styles.fileContainer}>
              <input
                ...
                onChange={handleFileChange}
              />
              {existingFiles.bookingFormFile && (
                <div
                  style={{ fontSize: "11px", color: "#555", marginTop: "4px" }}
                >
                  Current file: {existingFiles.bookingFormFile}
                </div>
              )}
              <span className={styles.fileInfo}>
          ...
          <div className={styles.fieldContainer}>
            <div className={styles.fileContainer}>
              <input
                ...
                onChange={handleFileChange}
              />
              {existingFiles.saleConfirmationDocFile && (
                <div
                  style={{ fontSize: "11px", color: "#555", marginTop: "4px" }}
                >
                  Current file: {existingFiles.saleConfirmationDocFile}
                </div>
              )}
              <span className={styles.fileInfo}>
                File format: .png, .gif, .jpeg, .webp, .jpg, .pdf
